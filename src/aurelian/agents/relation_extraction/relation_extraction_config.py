"""Configuration for the Relation Extraction Agent."""
...
@dataclass
class RelationExtractionDependencies(HasWorkdir):
    """Dependencies for the Relation Extraction Agent."""
    ...
    # Cache of extracted relations by file hash
    _relations_cache: Dict[str, List] = field(default_factory=dict)
    ...
    def __post_init__(self):
        ...
        if not self.cache_directory:
            self.cache_directory = os.path.join(self.workdir, ".relation_extraction_cache")
        ...
        if os.path.exists(cache_file):
            try:
                with open(cache_file, 'r') as f:
                    ...
                    self._relations_cache = cache_data.get("relations_cache", {})
    ...
    def save_cache(self):
        ...
        try:
            cache_data = {
                ...
                "relations_cache": self._relations_cache
            }
    ...
    def mark_as_processed(self, file_path: str, relations: List = None):
        """Mark a file as processed and cache its relations."""
        file_hash = self.get_file_hash(file_path)
        self._processed_hashes.add(file_hash)
        
        if relations:
            self._relations_cache[file_hash] = relations
        
        # Save cache after each update
        self.save_cache()
    
    def get_cached_relations(self, file_path: str) -> Optional[List]:
        """Get cached relations for a file if available."""
        file_hash = self.get_file_hash(file_path)
        return self._relations_cache.get(file_hash)
